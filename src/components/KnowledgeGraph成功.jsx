...
const KnowledgeGraph = () => {
    ...
    //     // 3. 提取这些边涉及到的所有节点（确保关系两端的节点都在图中）
    //     const nodesInLinks = new Set();
    //     relevantLinks.forEach(link => {
    //         nodesInLinks.add(typeof link.source === 'object' ? link.source.id : link.source);
    //         nodesInLinks.add(typeof link.target === 'object' ? link.target.id : link.target);
    //     });

    //     // 4. 构建最终显示的节点集（匹配到的点 + 它们的邻居点）
    //     const finalNodes = graphData.nodes.filter(node => nodesInLinks.has(node.id));
