...
const KnowledgeGraph = () => {
    ...
    const loadGraphData = async () => {
        try {
            ...
            // 后端返回的是图谱列表，每个图谱包含 triplets 数组
            graphs.forEach(graph => {
                graph.triplets.forEach((triplet, index) => {
                    if (!nodes.has(triplet.subject)) {
                        nodes.set(triplet.subject, { id: triplet.subject, name: triplet.subject, color: '#3B82F6' });
                    }
                    if (!nodes.has(triplet.object)) {
                        nodes.set(triplet.object, { id: triplet.object, name: triplet.object, color: '#10B981' });
                    }
                    links.push({
                        source: triplet.subject,
                        target: triplet.object,
                        relation: triplet.predicate,
                        id: `${graph.id}-${index}`, // 组合唯一ID
                        graphId: graph.id,
                        index: index
                    });
                });
            ...
            const data = { nodes: Array.from(nodes.values()), links: links };
            setGraphData(data);
    ...
    const handleSearch = () => {
        ...
        // 修复点：增加对 node.name 的安全检查
        const filteredNodes = graphData.nodes.filter(node =>
            node.name?.toLowerCase().includes(query)
        );

        const filteredLinks = graphData.links.filter(link => {
            // 修复点：兼容 source/target 为对象或字符串的情况
            const sourceName = (typeof link.source === 'object' ? link.source.id : link.source)?.toLowerCase();
            const targetName = (typeof link.target === 'object' ? link.target.id : link.target)?.toLowerCase();
            const relationName = link.relation?.toLowerCase();

            return sourceName.includes(query) || targetName.includes(query) || relationName.includes(query);
        });
    ...
    const saveEdit = async () => {
        try {
            // 构建请求体
            const payload = {
                triplets: editMode === 'node'
                    ? filteredGraphData.links.map(link => ({
                        ...link,
                        subject: link.subject === selectedNode.id ? editForm.subject : link.subject,
                        object: link.object === selectedNode.id ? editForm.subject : link.object,
                        predicate: link.relation
                    }))
                    : [{ subject: editForm.subject, predicate: editForm.predicate, object: editForm.object }]
            };
    ...
    const deleteLink = async (linkId) => {
        ...
        // 解析之前在 loadGraphData 中拼凑的 ID: `${graph.id}-${index}`
        const [graphId, index] = linkId.split('-');

        try {
            const response = await fetch(`${API_BASE_URL}/link/${graphId}/${index}`, {
                method: 'DELETE'
            });
                    ...
                    {extractedTriplets.length > 0 && (
                        <div className="bg-white rounded-lg shadow p-6">
                            <div className="flex justify-between items-center mb-4">
                                <h3 className="text-lg font-semibold">提取的三元组</h3>
                                <button
                                    onClick={saveGraph}
                                    ...
                                    <div key={op.id} className="flex justify-between items-center p-2 bg-gray-50 rounded border-l-4 border-blue-500">
                                        <div className="flex flex-col">
                                            <span className="text-sm font-medium">
                                                {op.operation_type === 'delete' ? '🔴 删除' : '🟢 增加'}{op.entity_type === 'link' ? '关系' : '节点'}
                                            </span>
